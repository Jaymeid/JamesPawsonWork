...
void ARiseDoor::OpenDoor(float dt)
{
	DoorCurrentLocation = this->GetActorLocation();
	/*
	...
	*/
	...
	if (CurrentRise < 35) {
		CurrentRise++;
		this->SetActorLocation(FVector(DoorCurrentLocation.X, DoorCurrentLocation.Y, (DoorCurrentLocation.Z + CurrentRise)));
	}
}

void ARiseDoor::CloseDoor(float dt)
{
	DoorCurrentRotation = Door->GetRelativeRotation().Yaw;
...
void ARiseDoor::ToggleDoor(FVector ForwardVector)
{
